<template>
  <div class="page-container">
    <PageHeader title="地域マスタ" subtitle="Region Master" />
    ...
    <el-card class="search-card search-form-grid-card search-form-single-row-card master-list-search-card">
      <el-form :model="query" label-width="84px" class="search-form-grid search-form-single-row">
        <div class="search-form-grid__cols">
          <el-form-item label="地域名称" class="search-field-name">
    ...
    <el-card v-loading="loading" class="table-card">
      ...
      <el-table :data="tableData" class="data-table" border stripe empty-text="データがありません">
        ...
        <el-table-column prop="regionId" label="地域ID" min-width="140" />
        <el-table-column prop="code" label="地域コード" min-width="140" />
        <el-table-column prop="name" label="地域名称" min-width="200" />
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="480px" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="110px" class="dialog-form">
        <el-form-item label="地域コード" prop="code">
...
import { createRegion, deleteRegion, getRegions, updateRegion } from '@/api/region'
...
const rules = {
  code: [{ required: true, message: '地域コードを入力してください', trigger: 'blur' }],
  name: [{ required: true, message: '地域名称を入力してください', trigger: 'blur' }]
}

const dialogTitle = computed(() => (form.regionId ? '地域編集' : '地域新規登録'))
...
async function handleSubmit() {
  ...
  try {
    ...
    if (form.regionId) {
      await updateRegion(form.regionId, payload)
      ElMessage.success('地域を更新しました')
    } else {
...
async function handleDelete(row) {
  await ElMessageBox.confirm(`地域「${row.name}」を削除しますか？`, '確認', { type: 'warning' })
  await deleteRegion(row.regionId)
